 ...
 * 这个宏就是针对编译环境做一些检查 如果检查不通过则自己产生一个编译错误使用typedef定义一个数组长度为-1的数组类型
 __COUNTER__ 这个宏应该是用于计数的 
 */
...
/* 如果必选字段小于64 则输出错误信息 必选字段不应该小于64 */
#if PB_MAX_REQUIRED_FIELDS < 64
#error You should not lower PB_MAX_REQUIRED_FIELDS from the default value (64).
#endif

/* 列出可能的字段类型      这些字段类型会在自动生成的代码中使用     */
/* List of possible field types. These are used in the autogenerated code.
 * 最低4位代表了数量类型
 * Least-significant 4 bits tell the scalar type
 * 最高4位指定了字段是否必选 可重复等属性
...
/* Numeric types */
#define PB_LTYPE_VARINT  0x00 /* int32, int64, enum, bool 这个可以理解为有符号的整数*/ 
#define PB_LTYPE_UVARINT 0x01 /* uint32, uint64 无符号整数*/
#define PB_LTYPE_SVARINT 0x02 /* sint32, sint64 */
#define PB_LTYPE_FIXED32 0x03 /* fixed32, sfixed32, float */
#define PB_LTYPE_FIXED64 0x04 /* fixed64, sfixed64, double */

/* Marker for last packable field type. 标记最后一个压缩字段类型 */
#define PB_LTYPE_LAST_PACKABLE 0x04

/* Byte array with pre-allocated buffer.
 * data_size is the length of the allocated PB_BYTES_ARRAY structure. */
#define PB_LTYPE_BYTES 0x05 /* byte数组 */

/* String with pre-allocated buffer.
 * data_size is the maximum length. */
#define PB_LTYPE_STRING 0x06 /* 字符串 */

/* Submessage  消息结构中 有一个子结构体
 * submsg_fields is pointer to field descriptions */
#define PB_LTYPE_SUBMESSAGE 0x07

/* Extension pseudo-field 消息结构中 有一个扩展子结构体
...
#define PB_LTYPE(x) ((x) & PB_LTYPE_MASK)  /* 0000 1111 */
